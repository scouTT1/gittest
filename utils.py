...
def verify_checkpoint_dir(checkpoint_dir, resume, test_mode):
    ...
    #        sys.exit()
        ...
        return len(os.listdir(checkpoint_dir))
...
def get_log_files(checkpoint_dir, resume, test_mode):
    ...
    """
    ckpt_idx = verify_checkpoint_dir(checkpoint_dir, resume, test_mode)
    #if not test_mode and not resume:
    checkpoint_dir = os.path.join(checkpoint_dir, f"ckpt{ckpt_idx}")
    if not resume:
